...
namespace LitJson
{
    ...
    public class JsonMapper
    {
            ...
            if (reader.Token == JsonToken.ArrayStart) {
                ...
                while (true) {
                    ...
                    if (t_data.Properties.ContainsKey (property)) {
                        ...
                        try
                        {
                            var isJsonData = prop_data.Type.Name.Equals("JsonData");
                            var valueObject = isJsonData ? ToObject(reader) : ReadValue(prop_data.Type, reader);
                            if (prop_data.IsField)
                            {
                                ((FieldInfo) prop_data.Info).SetValue(instance, valueObject);
                            }
                            else
                            {
                                PropertyInfo p_info =
                        ...
                        {
                            throw new JsonException($"{prop_data.Info.Name}: {e.Message} readTimes:{readTimes} property:{property}");
                        }
        ...
        public static object ToObject(string json, Type inst_type)
        {
            ...
            return ReadValue (inst_type, reader);
        }
